...
@php
    $details=DB::table('blockpartners')->where('id',$id)->first();
@endphp
<div class="main-content">
    ...
    <h4 class="card-title mb-0 flex-grow-1">Update Block Partner </h4>
    ...
    @endif
                        ...
                        <div class="card-body">
                            <div class="live-preview">
                                <form action="{{url('/admin/blockpartner/update')}}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="row gy-4">
                                        ...
                                        <div class="col-xxl-3 col-md-4">
                                            <div>
                                                <label for="placeholderInput" class="form-label">Block*</label>
                                                <select class="form-control" name="block_id" required>
                                                    <option value="">Select Block</option>
                                                    @php
                                                    $district_partner_row=DB::table('districtpartners')->where('id',$details->district_partner_id)->first();
                                                    $block_row=DB::table('block')->where('district_id',$district_partner_row->district_id)->get();
                                                    @endphp
                                                    @foreach ($block_row as $block_details)
                                                        <option value="{{ $block_details->id }}" {{ $details->block_id==$block_details->id?'selected':'' }}>{{ $block_details->block }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
